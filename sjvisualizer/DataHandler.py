...
class DataHandler():
    ...
    def _prep_data(self):
        ...
        for i, (index, row) in enumerate(self.df.iterrows()):
            ...
            if i > 0:
                ...
                while time < index:
                    ...
                    #temp_df = pd.concat([temp_df, pd.DataFrame(None, index=[time], columns=self.df.columns)], axis=0)
                    time = time + self.dt
        ...
        for i in range(1, 60*10):
            ...
            #temp_df = pd.concat([temp_df, pd.DataFrame([list(row)])], axis=0, index=[time], columns=self.df.columns)
